"""
RQ2 Plotting Script: FWHT vs DFS Comparison (Sliding-N Strategy)
================================================================
...
# ============================================================================
# 1. Raw Data  (FWHT from 022301.exe, DFS from euler_dfs.exe)
# ============================================================================

# --- FWHT results (from C++ OpenMP runs) ---
# Format: filename -> (N, M, genus, time_s, circuits)
fwht_data = {
    # Sparse
...
# --- DFS results (from euler_dfs.exe CSV) ---
dfs_csv_path = r"D:\cpp\.vscode\eulerian_RQ2_DFS_results.csv"
dfs_data = {}
...
# Build merged records: only where BOTH FWHT and DFS exist
records = []
...
# ============================================================================
# Empirical fact: for OK entries, total_dfs_raw = dfs_circuits * 2m
# Compute per-family median ratio: total_dfs_raw / fwht_circuits
from collections import defaultdict
family_ratios = defaultdict(list)
for r in records:
    if r["dfs_status"] == "OK" and r["fwht_circuits"] > 0 and r["dfs_circuits"] > 0:
...
for fam, ratios in family_ratios.items():
    ...
    print(f"  [Ratio] {fam}: median(dfs_raw / fwht_circuits) = {family_avg_ratio[fam]:.3f}")
...
for r in records:
    if r["dfs_status"] == "TIMEOUT":
        ...
        print(f"  [Extrap] {r['file']}: est. DFS = {r['dfs_extrapolated']:.1f}s "
...
# ============================================================================
FAMILY_STYLE = {
    "Sparse":  {"color": "#2196F3", "marker": "s", "label": "Sparse (N=15\u201330)"},
    "Medium":  {"color": "#FF9800", "marker": "o", "label": "Medium (N=8\u201310)"},
    "Dense":   {"color": "#E91E63", "marker": "D", "label": "Dense (N=6\u20137)"},
    "Tangled": {"color": "#9C27B0", "marker": "^", "label": "Tangled (N=7)"},
}
...
output_dir = r"D:\cpp\.vscode\RQ2_figures"
os.makedirs(output_dir, exist_ok=True)

# ============================================================================
# Figure 1: FWHT vs DFS Runtime vs Genus (Main Result)
# ============================================================================
fig1, ax1 = plt.subplots(figsize=(12, 7))

for fam in ["Sparse", "Medium", "Dense", "Tangled"]:
    sty = FAMILY_STYLE[fam]
    sub = sorted([r for r in records if r["family"] == fam], key=lambda x: x["genus"])
    if not sub:
        continue
    ...
    dfs_to_g = [r["genus"]           for r in sub if r["dfs_status"] == "TIMEOUT"]
    dfs_to_t = [r["dfs_extrapolated"] for r in sub if r["dfs_status"] == "TIMEOUT"]
    ...
    if dfs_ok_g:
        ax1.plot(dfs_ok_g, dfs_ok_t, color=sty["color"], marker=sty["marker"],
                 markersize=8, linewidth=2, linestyle="--", alpha=0.85, zorder=4,
                 markerfacecolor="none", markeredgewidth=2)

    # DFS TIMEOUT (extrapolated): large X with upward arrow from 600s
    if dfs_to_g:
        ax1.scatter(dfs_to_g, dfs_to_t, color=sty["color"], marker="X", s=160,
        ...
        # Connect last OK point to first extrapolated point
        if dfs_ok_g:
            ax1.plot([dfs_ok_g[-1], dfs_to_g[0]], [dfs_ok_t[-1], dfs_to_t[0]],
                     color=sty["color"], linestyle=":", linewidth=1.2, alpha=0.4)
...
ax1.set_ylabel("Computation Time (s, log scale)", fontsize=13, fontweight="bold")
ax1.set_title("RQ2: FWHT vs DFS \u2014 Runtime vs Graph Genus\n"
              "(Solid = FWHT, Dashed = DFS, \u2715 = DFS Extrapolated from Timeout)",
...
for fam in ["Sparse", "Medium", "Dense", "Tangled"]:
    sty = FAMILY_STYLE[fam]
    legend_el.append(Line2D([0], [0], color=sty["color"], marker=sty["marker"],
...
legend_el.append(Line2D([0], [0], color="black", marker="X", markersize=10,
                         linestyle="None", label="DFS Timeout (extrapolated)"))
legend_el.append(Line2D([0], [0], color="red", linestyle=":", linewidth=1.5,
                         label="600 s Timeout Limit"))

ax1.legend(handles=legend_el, loc="upper left", fontsize=7.5, ncol=2,
           framealpha=0.9, edgecolor="gray")
fig1.tight_layout()
fig1.savefig(os.path.join(output_dir, "RQ2_fig1_fwht_vs_dfs_genus.png"),
...
for fam in ["Sparse", "Medium", "Dense", "Tangled"]:
    ...
    g_ok, s_ok, g_ex, s_ex = [], [], [], []
    for r in sub:
        sp = r["dfs_extrapolated"] / r["fwht_time"] if r["fwht_time"] > 0 else 0
        if sp <= 0:
            continue
        if r["dfs_status"] == "OK":
    ...
    if g_ex:
        ...
        if g_ok:
            ax2.plot([g_ok[-1]] + g_ex, [s_ok[-1]] + s_ex,
                     color=sty["color"], linestyle=":", linewidth=1.5, alpha=0.5)
...
ax2.set_ylabel("Speedup  (DFS Time / FWHT Time)", fontsize=13, fontweight="bold")
ax2.set_title("RQ2: FWHT Speedup over DFS\n"
              "(\u2715 = Extrapolated from 600 s Timeout)",
              fontsize=14, fontweight="bold")

# Annotate maximum speedup
all_sp = [(r["genus"], r["dfs_extrapolated"] / r["fwht_time"], r["family"], r["file"])
          for r in records if r["fwht_time"] > 0 and r["dfs_extrapolated"] > 0]
if all_sp:
...
for fam in ["Sparse", "Medium", "Dense", "Tangled"]:
    sty = FAMILY_STYLE[fam]
    legend_el2.append(Line2D([0], [0], color=sty["color"], marker=sty["marker"],
                             markersize=8, linewidth=2.5, label=sty["label"]))
legend_el2.append(Line2D([0], [0], color="black", marker="X", markersize=10,
                          linestyle="None", label="Extrapolated"))
ax2.legend(handles=legend_el2, loc="upper left", fontsize=10, framealpha=0.9)
fig2.tight_layout()
fig2.savefig(os.path.join(output_dir, "RQ2_fig2_speedup.png"),
             dpi=300, bbox_inches="tight")
print("[Saved] Fig 2")

# ============================================================================
# Figure 3: Runtime vs Edge Count (m) - Both Methods
# ============================================================================
fig3, ax3 = plt.subplots(figsize=(12, 7))

for fam in ["Sparse", "Medium", "Dense", "Tangled"]:
    sty = FAMILY_STYLE[fam]
    sub = sorted([r for r in records if r["family"] == fam], key=lambda x: x["M"])
    if not sub:
        continue
    ...
    m_to = [r["M"]               for r in sub if r["dfs_status"] == "TIMEOUT"]
    t_to = [r["dfs_extrapolated"] for r in sub if r["dfs_status"] == "TIMEOUT"]
    if m_to:
...
ax3.set_ylabel("Computation Time (s, log scale)", fontsize=13, fontweight="bold")
...
ax3.legend(handles=legend_el, loc="upper left", fontsize=7.5, ncol=2,
           framealpha=0.9, edgecolor="gray")
fig3.tight_layout()
fig3.savefig(os.path.join(output_dir, "RQ2_fig3_time_vs_edges.png"),
             dpi=300, bbox_inches="tight")
print("[Saved] Fig 3")
...
sorted_recs = sorted(records,
                     key=lambda r: (family_order.get(r["family"], 9), r["genus"]))

for r in sorted_recs:
    fwht_s = f"{r['fwht_time']:.4f}" if r["fwht_time"] < 1 else f"{r['fwht_time']:.2f}"
    ...
    if r["dfs_status"] == "TIMEOUT":
        ...
        sp_s = f"\u2248{sp:,.0f}\u00d7"
        row_c = ["#FFCDD2"] * 9          # red highlight
    else:
        dfs_s = f"{r['dfs_time']:.4f}" if r["dfs_time"] < 1 else f"{r['dfs_time']:.2f}"
        status_s = "OK"
        sp = r["dfs_time"] / r["fwht_time"] if r["fwht_time"] > 0 else 0
        sp_s = f"{sp:,.1f}\u00d7" if sp < 1000 else f"{sp:,.0f}\u00d7"
        if sp > 100:
            row_c = ["#C8E6C9"] * 9      # green
        elif sp > 10:
            row_c = ["#FFF9C4"] * 9      # yellow
        else:
            row_c = ["#FFFFFF"] * 9      # white
...
ax4.set_title("RQ2: Complete Experimental Data \u2014 FWHT vs DFS\n"
              "(Red = DFS Timeout & Extrapolated, Green = Speedup > 100\u00d7)",
              fontsize=14, fontweight="bold", pad=20)

fig4.tight_layout()
fig4.savefig(os.path.join(output_dir, "RQ2_fig4_data_table.png"),
             dpi=300, bbox_inches="tight")
print("[Saved] Fig 4")
...
if ok_sp:
    ...
    print(f"  Max OK speedup:  {max(vals):.1f}x  ({max(ok_sp,key=lambda x:x[0])[1]})")
