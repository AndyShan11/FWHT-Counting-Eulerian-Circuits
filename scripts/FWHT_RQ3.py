"""
RQ3: Generate Eulerian graphs at FIXED circuit rank g with varying edge count M.
Three graph families:
  1. Cactus  — K cycles sharing only cut-vertices (g = K)
  2. Chain-of-cycles — cycles linked sequentially (g = K)
  3. Random-Augmented-Tree — start from a tree, add g random back-edges,
     then repair parity by adding parallel edges
All output in DIMACS .gr format.
"""
import random
import os
import sys
...
# ---------------------------------------------------------------------------
# Family 1: Cactus graph  (K cycles on disjoint new vertices, attached to tree)
# ---------------------------------------------------------------------------
def generate_cactus(M, g):
    """
    Build a cactus with exactly g cycles whose total edge count is M.
    g = number of cycles => circuit rank = g.
    V = M - g + 1.
    """
    K = g
    if M < 3 * K:
...
# ---------------------------------------------------------------------------
# Family 2: Chain of cycles (sequential)
# ---------------------------------------------------------------------------
def generate_chain_of_cycles(M, g):
    """
    Build g cycles linked sequentially (last node of cycle i = first node of cycle i+1).
    Circuit rank = g.
    """
    K = g
    if M < 3 * K:
    ...
    edges = []
    cur_id = 1  # first node of next cycle

    for ci, length in enumerate(cycle_lengths):
        start_node = cur_id
        # create 'length' new nodes (but the first one reuses cur_id)
        nodes_in_cycle = [start_node + j for j in range(length)]
        for i in range(length):
            u = nodes_in_cycle[i]
            v = nodes_in_cycle[(i + 1) % length]
            edges.append((u, v))
        # next cycle starts at the last node of this cycle
        if ci < K - 1:
...
# ---------------------------------------------------------------------------
# Family 3: Random Eulerian graph with controlled g
# ---------------------------------------------------------------------------
def generate_random_eulerian(M, g):
    """
    Start from a path (tree) on n = M - g + 1 nodes,
    add g random back-edges to create circuit rank g,
    then ensure all degrees are even by adding parallel edges.
    Final edge count may slightly exceed M — we trim/retry to match.
    """
    n = M - g + 1
    if n < 3:
    ...
    for _ in range(max_tries):
        # spanning tree: path 1-2-3-...-n
        edges = []
        for i in range(1, n):
            edges.append((i, i + 1))

        # add g random back-edges
        added = 0
        attempts = 0
        ...
        # fix parity: for each odd-degree node, add a parallel edge to a neighbor
        ...
        # pair up odd-degree nodes and add an edge between each pair
        ...
        # we accept if g matches; actual_m may differ from target M
        if actual_g == g:
            return n, actual_m, edges

    # fallback to cactus if random fails
...
if __name__ == "__main__":
    ...
    # For each g, generate graphs with increasing M
    # M must satisfy M >= 3g (minimum for cactus) and n = M - g + 1 >= 3
    M_RANGES = {
    ...
    output_dir = "RQ3_graphs"
    csv_output = r"D:\cpp\.vscode\eulerian_RQ3_results.csv"
    os.makedirs(output_dir, exist_ok=True)

    print(f"[*] Graph output dir : {output_dir}")
    print(f"[*] Expected CSV name: {csv_output}")
    print()
    print(f"{'Family':<10} {'g':<4} {'M_target':<10} {'V':<6} {'M_actual':<10} {'file'}")
    print("-" * 70)
            ...
            for fam_name, gen_fn in FAMILIES.items():
                for rep in range(1, REPEATS + 1):
                    try:
                        ...
                        print(f"{fam_name:<10} {g:<4} {m_target:<10} {V:<6} {E:<10} {fname}")
                    except Exception as ex:
                        print(f"{fam_name:<10} {g:<4} {m_target:<10} SKIP ({ex})")
