"""
RQ4: Parallel Scaling Analysis
Reads rq4_results.csv and produces publication-quality figures:
  Fig 1: Speedup vs Threads for multiple problem sizes (with ideal linear reference)
...
# ============================================================
# Compute speedup and efficiency for each graph
# ============================================================
all_threads = sorted(set(t for gd in data.values() for t in gd.keys()))

graph_stats = {}  # graph -> {threads: (speedup_mean, speedup_std, eff_mean, eff_std, time_mean)}
...
# ============================================================
# Fig 1: Speedup vs Threads  (bar groups + ideal line)
# ============================================================
...
for gi, gname in enumerate(valid_graphs):
    ...
    offsets = x_pos[:len(thrs)] + gi * bar_width - (n_graphs - 1) * bar_width / 2
    ...
    for i, bar in enumerate(bars):
        ...
        ax1.text(bar.get_x() + bar.get_width() / 2, yval + 0.15,
                 f'{yval:.1f}x', ha='center', va='bottom', fontsize=7, fontweight='bold')
        if t_mean >= 0.01:
            ax1.text(bar.get_x() + bar.get_width() / 2, yval * 0.4,
                     f'{t_mean:.2f}s', ha='center', va='center', fontsize=6,
                     color='white', fontweight='bold')

# Ideal linear scaling line
max_threads_idx = len(all_threads)
ideal = [float(t) for t in all_threads]
ax1.plot(x_pos[:max_threads_idx], ideal[:max_threads_idx],
         'k--', linewidth=2, alpha=0.5, marker='', label='Ideal Linear Scaling')
...
fig1.tight_layout()
fig1.savefig('RQ4_speedup.png', dpi=300, bbox_inches='tight')
fig1.savefig('RQ4_speedup.pdf', bbox_inches='tight')
print("[+] Saved RQ4_speedup.png / .pdf")

# ============================================================
# Fig 2: Parallel Efficiency (auto-scaled, with superlinear annotation)
# ============================================================
...
ax2.axhline(y=1.0, color='gray', linestyle='--', linewidth=1.5, alpha=0.6, label='Ideal (100%)')
...
y_max = max(y_max, 1.2)  # at least show ideal line
...
ax2.set_ylabel('Parallel Efficiency (Speedup / Threads)', fontsize=13, fontweight='bold')
...
fig2.tight_layout()
fig2.savefig('RQ4_efficiency.png', dpi=300, bbox_inches='tight')
fig2.savefig('RQ4_efficiency.pdf', bbox_inches='tight')
print("[+] Saved RQ4_efficiency.png / .pdf")

# ============================================================
# Fig 3: Amdahl's Law Fit
# Amdahl: S(p) = 1 / (f + (1-f)/p)  where f = serial fraction
# ============================================================
...
# Simple manual Amdahl fit (avoid scipy dependency)
...
for gi, gname in enumerate(valid_graphs):
    ...
    try:
        ...
        # Annotate serial fraction
        m_match = g_pattern.match(gname)
        g_val = m_match.group(1) if m_match else '?'
        ax3.annotate(f'$f={f_serial:.3f}$\n($g={g_val}$)',
                     xy=(max(all_threads) * 0.85, amdahl(max(all_threads), f_serial)),
                     fontsize=8, color=COLORS[gi % len(COLORS)],
                     ha='center',
...
ax3.plot(p_ideal, p_ideal, 'k--', linewidth=1.5, alpha=0.4, label='Ideal ($f=0$)')
...
fig3.tight_layout()
fig3.savefig('RQ4_amdahl.png', dpi=300, bbox_inches='tight')
fig3.savefig('RQ4_amdahl.pdf', bbox_inches='tight')
...
print("=" * 80)
print(f"{'Graph':<25} {'Threads':<10} {'Time(s)':<12} {'Speedup':<10} {'Efficiency':<12}")
print("-" * 80)
for gname in valid_graphs:
    stats = graph_stats[gname]
    for thr in sorted(stats.keys()):
        s = stats[thr]
        print(f"{gname:<25} {thr:<10} {s['time_mean']:<12.4f} {s['speedup_mean']:<10.2f} {s['eff_mean']:<12.3f}")
