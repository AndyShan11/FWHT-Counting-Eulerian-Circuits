"""
RQ3: Plot — Effect of Graph Size (M) at Fixed Circuit Rank (g)
Reads the CSV produced by 022301.cpp, generates publication-quality figures.

Figures produced:
  Fig 1 (main):  T vs M for g=8,10,12 with O(m^3 log m) fit, error bars, 3 families
  Fig 2 (sub):   T / (m^3 log m)  normalized — should be flat if complexity is correct
  Fig 3 (sub):   Eulerian circuit count vs M at each g
"""
...
# 0. Load data
# ============================================================
CSV_FILE = r"D:\cpp\.vscode\eulerian_RQ3_results.csv"
...
with open(CSV_FILE, 'r') as f:
    ...
    for line in f:
        ...
        if pm:
            ...
            # Scale partial count by completion ratio for estimation
...
LABELS_FAM = {'cactus': 'Cactus', 'chain': 'Chain-of-Cycles', 'random': 'Random Eulerian'}
...
for idx, g in enumerate(sorted(data.keys())):
    ...
    # Aggregate by family, separating complete vs partial
    ...
    # Plot each family: solid for complete, dashed for partial/predicted
    for fam in ['cactus', 'chain', 'random']:
        ...
        if fam in fam_complete and fam_complete[fam]:
            ...
            line = ax.errorbar(Ms, means, yerr=stds,
                         ...
                         label=LABELS_FAM.get(fam, fam),
                         alpha=0.85)
            color = line[0].get_color()

        # Partial data — dashed line with hollow markers
        if fam in fam_partial and fam_partial[fam]:
            ...
            lbl = LABELS_FAM.get(fam, fam) + ' (predicted)' if color is None else None
    ...
    # Fit T = C * m^3 * log(m) to COMPLETE data only at this g
    all_M = []
    all_T = []
    for (fam, M), vals in gdata.items():
        for t, ec, is_partial in vals:
...
fig.tight_layout()
fig.savefig('RQ3_time_vs_M.png', dpi=300, bbox_inches='tight')
fig.savefig('RQ3_time_vs_M.pdf', bbox_inches='tight')
print("[+] Saved RQ3_time_vs_M.png / .pdf")

# ============================================================
# 2. Normalized plot: T / (m^3 log m) should be roughly constant
# ============================================================
...
for idx, g in enumerate(sorted(data.keys())):
    ...
    for fam in ['cactus', 'chain', 'random']:
        ...
        complete_M, partial_M = defaultdict(list), defaultdict(list)
...
fig2.suptitle('RQ3: Normalized Time (Complexity Validation)',
              fontsize=15, fontweight='bold', y=1.02)
fig2.tight_layout()
fig2.savefig('RQ3_normalized.png', dpi=300, bbox_inches='tight')
fig2.savefig('RQ3_normalized.pdf', bbox_inches='tight')
...
for idx, g in enumerate(sorted(data.keys())):
    ...
    for fam in ['cactus', 'chain', 'random']:
        ...
        for M, vals in fam_items.items():
            for t, ec, is_partial in vals:
                if is_partial:
                    partial_M[M].append(ec)  # already scaled by completion ratio
                else:
    ...
    ax.set_yscale('log')
    ax.set_title(f'$g = {g}$', fontsize=13, fontweight='bold')
    # Add custom legend entry for dashed = predicted
...
fig3.suptitle('RQ3: Eulerian Circuit Count vs. Graph Size',
              fontsize=15, fontweight='bold', y=1.02)
fig3.tight_layout()
fig3.savefig('RQ3_ec_count.png', dpi=300, bbox_inches='tight')
fig3.savefig('RQ3_ec_count.pdf', bbox_inches='tight')
