...
\maketitle
	...
	\textbf{Experimental setup.} In addition to the synthetic graphs used in RQ1--RQ4, we evaluate both solvers on 61 real-world graphs drawn from molecular chemistry datasets and infrastructure networks (RQ5). All graphs were preprocessed to satisfy the Eulerian condition by adding a minimum set of edges to make all vertex degrees even; this augmentation increases the circuit rank by at most the number of added edges. The DFS baseline uses bridge-aware backtracking (Fleury pruning): at each step, Tarjan's $\bigO(n+m)$ bridge-detection algorithm identifies edges whose removal would disconnect the remaining graph, and the solver branches only on non-bridge edges. A bridge is taken only when it is the sole remaining edge from the current vertex; the implications for the crossover analysis are discussed in Section~\ref{subsec:crossover}.
	...
	\textbf{Strength of the DFS baseline.} The backtracking baseline incorporates Fleury-style bridge pruning via Tarjan's algorithm, which eliminates provably dead-end branches at each step. This is a meaningful optimization: at low circuit rank, most edges are bridges (the spanning tree dominates), so bridge pruning dramatically reduces the branching factor. At high circuit rank, fewer edges are bridges, and the pruning benefit diminishes---precisely the regime where FWHT already dominates. Consequently, the crossover analysis in RQ2 and RQ5 is robust to reasonable improvements in the DFS baseline. Further strengthening via intelligent edge ordering or partial memoization could shift the crossover by 1--2 units of $g$ but would not change the qualitative picture.
	
	\textbf{Correctness verification.} We verified the implementation against brute-force enumeration on all graphs with $g \le 8$ in our test suite, and additionally confirmed consistency between the FWHT and DFS solvers on all 61 real-world instances where DFS completed (the raw DFS trail counts satisfy $\text{raw} = \ec(G) \cdot d(v_0)$, where $v_0$ is the starting vertex). For larger~$g$, independent verification via MCMC estimates~\cite{CreedGoldberg2013} or combinatorial identities for specific graph families remains an important direction.
